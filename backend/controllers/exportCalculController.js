...
exports.exporterCalculPDF = async (req, res, next) => {
    ...
    try {
        ...
        const result = await ExportService.exportPDF(data, {
            type: type_rapport,
        ...
        if (result.signature) {
            res.setHeader('X-Signature', result.signature);
            res.setHeader('X-Signed', 'true');
        }
...
exports.exporterCalculWord = async (req, res, next) => {
    ...
    try {
        ...
        const result = await ExportService.exportWord(data, {
            type: type_rapport,
            title: 'Rapport de calcul'
        });

        console.log('Word genere, taille:', result.buffer.length);
