...
exports.getAllProduits = (req, res) => {
  db.query(
    'SELECT * FROM produits WHERE entreprise_id = ? ORDER BY id DESC',
...
exports.getProduitById = (req, res) => {
  db.query(
    'SELECT * FROM produits WHERE id = ? AND entreprise_id = ?',
...
exports.createProduit = (req, res) => {
  ...
  const sql = 'INSERT INTO produits (entreprise_id, nom, description, prix, quantite_stock) VALUES (?, ?, ?, ?, ?)';
  db.query(sql, [req.user.entreprise_id, nom.trim(), description || null, Number(prix), stock], (err, result) => {
    if (err) { console.error(err); return res.status(500).json({ message: 'Erreur serveur' }); }
...
exports.updateProduit = (req, res) => {
  ...
  const sql = 'UPDATE produits SET nom = ?, description = ?, prix = ?, quantite_stock = ? WHERE id = ? AND entreprise_id = ?';
  db.query(sql, [nom.trim(), description || null, Number(prix), stock, req.params.id, req.user.entreprise_id], (err, result) => {
    if (err) { console.error(err); return res.status(500).json({ message: 'Erreur serveur' }); }
...
exports.deleteProduit = (req, res) => {
  db.query(
    'DELETE FROM produits WHERE id = ? AND entreprise_id = ?',
    [req.params.id, req.user.entreprise_id],
    (err, result) => {
      if (err) { console.error(err); return res.status(500).json({ message: 'Erreur serveur' }); }
      if (result.affectedRows === 0) return res.status(404).json({ message: 'Produit introuvable' });
