...
export default function PaiementClient() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.commande_id || !form.montant || form.montant <= 0) {
      setError('Veuillez saisir une commande et un montant valide');
      setLoading(false);
      return;
    }

    try {
      const response = await API.post('/paiement/create', {
        commande_id: parseInt(form.commande_id),
        montant: parseFloat(form.montant),
        description: form.description || `Paiement commande #${form.commande_id}`,
      });
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h1 style={styles.title}>Paiement en ligne</h1>
        <p style={styles.subtitle}>Payez votre commande en toute securite</p>
      </div>
