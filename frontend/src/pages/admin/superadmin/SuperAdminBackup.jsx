...
export default function SuperAdminBackup() {
    ...
    const triggerBackup = async () => {
        if (!window.confirm('Lancer une sauvegarde manuelle de toutes les bases de donnees ?')) return;
        setBackupLoading(true);
        setMessage('');
        setError('');
        try {
            const res = await API.post('/superadmin/backup');
            setMessage(res.data.total + ' bases de donnees sauvegardees avec succes');
            loadBackups();
        } catch (err) {
    ...
    const cleanupBackups = async () => {
        const days = prompt('Nombre de jours a conserver (defaut: 30) :', '30');
        if (days === null) return;
        ...
        try {
            ...
            setMessage(res.data.deleted + ' sauvegardes supprimees');
            loadBackups();
        } catch (err) {
    ...
    const columns = [
        ...
        { key: 'dbName', label: 'Base de donnees' },
        {
            key: 'filename',
            label: 'Fichier',
            render: (row) => (
    ...
    return (
        <div>
            <div style={styles.header}>
                <div>
                    <h1 style={styles.title}>Sauvegarde des donnees</h1>
                    <p style={styles.subtitle}>Gerez les sauvegardes automatiques de la plateforme</p>
                </div>
            ...
            <div style={styles.statsGrid}>
                ...
                <div style={{ ...styles.statCard, borderLeft: '4px solid #0EA5E9' }}>
                    ...
                    <span style={styles.statLabel}>Sauvegardes chiffrees</span>
                </div>
            </div>
            ...
            <Card title="Historique des sauvegardes" variant="primary" style={{ marginTop: '20px' }}>
                {backups.length === 0 ? (
                    <EmptyState
                        ...
                        description="Aucune sauvegarde n'a encore ete effectuee."
                    />
                ) : (
